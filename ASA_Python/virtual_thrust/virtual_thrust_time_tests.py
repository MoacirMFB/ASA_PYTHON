...
WARNING_TIME_SWEEP_MONTHS = (6, 12, 24, 36, 60)
_PLOT_IMAGE_EXPORT_WARNING_EMITTED = False
...
class VirtualThrustConfig:
    ...
    show_plots: bool = True         # Whether to open interactive figures for a single run
    print_scp_progress: bool = True  # Whether to print one SCP progress line per iteration
...
def _write_plotly_figure(fig: go.Figure, base_path: Path) -> None:
    """Save one Plotly figure to HTML and, when available, a PNG image."""

    global _PLOT_IMAGE_EXPORT_WARNING_EMITTED
    ...
    try:
        ...
        # Static image export needs an image backend such as Kaleido.
        if not _PLOT_IMAGE_EXPORT_WARNING_EMITTED:
            print(f"Static image export skipped; HTML plots were still saved. ({exc})")
            _PLOT_IMAGE_EXPORT_WARNING_EMITTED = True


def _save_figure_bundle(figures: dict[str, go.Figure], plot_dir: Path) -> None:
    """Save a named figure bundle into one folder."""

    for name, fig in figures.items():
        _write_plotly_figure(fig, plot_dir / name)
...
# ============================================================================
# Warning-time sweep helpers
# ============================================================================
# This section reuses the main single-run workflow and only varies `t0_months`
# so the sweep isolates warning time as the changed factor.

def _compute_control_usage_metrics(result: VirtualThrustRunResult) -> dict[str, float]:
    """Compute simple control-usage metrics from the optimized control history."""

    if result.scp is None or result.scp.U_opt is None or result.t_grid_s.size < 2:
        return {
            "max_control_norm": np.nan,
            "mean_control_norm": np.nan,
            "bang_bang_fraction": np.nan,
            "tau_used_s": np.nan,
            "tau_used_fraction": np.nan,
            "dv_used_mps": np.nan,
        }

    u_norm = np.linalg.norm(result.scp.U_opt, axis=1)
    max_control_norm = float(np.max(u_norm))
    mean_control_norm = float(np.mean(u_norm))
    dt_seg_s = float(result.t_grid_s[1] - result.t_grid_s[0])
    tau_used_s = float(np.sum(u_norm) * dt_seg_s)
    tau_used_fraction = float(tau_used_s / result.tau_budget_s) if result.tau_budget_s > 0.0 else np.nan
    dv_used_mps = float(1000.0 * result.amax_kmps2 * tau_used_s)
    if max_control_norm < 1e-12:
        bang_bang_fraction = 0.0
    else:
        bang_bang_fraction = float(np.mean(u_norm >= 0.9 * max_control_norm))

    return {
        "max_control_norm": max_control_norm,
        "mean_control_norm": mean_control_norm,
        "bang_bang_fraction": bang_bang_fraction,
        "tau_used_s": tau_used_s,
        "tau_used_fraction": tau_used_fraction,
        "dv_used_mps": dv_used_mps,
    }


def _write_sweep_csv(rows: list[dict[str, Any]], csv_path: Path) -> None:
    """Write the warning-time sweep table to CSV."""
...
def _build_sweep_metric_figure(
    ...
    *,
    ...
    """Build one simple summary line plot for the warning-time sweep."""

    x_vals = np.asarray([float(row["t0_months"]) for row in rows], dtype=float)
    y_vals = np.asarray([float(row[y_key]) for row in rows], dtype=float)
    ...
    fig.add_trace(
        go.Scatter(
            ...
            marker={"size": 9, "color": "#d95f02"},
            customdata=np.asarray([row["scp_success"] for row in rows], dtype=object).reshape(-1, 1),
            hovertemplate=(
                "Warning time = %{x:.0f} months<br>"
                f"{y_title} = %{{y:.6g}}<br>"
                "SCP success = %{customdata[0]}<extra></extra>"
            ),
        )
    )
    _add_info_box(fig, info_lines)
    fig.update_layout(
        title=title,
        xaxis_title="Warning time [months]",
        yaxis_title=y_title,
        template="plotly_white",
    )
    return fig
...
def _print_warning_time_summary(rows: list[dict[str, Any]]) -> None:
    """Print a short trend summary from the computed sweep metrics."""

    if not rows:
        print("Warning-time sweep summary: no runs were recorded.")
        return

    ordered_rows = sorted(rows, key=lambda row: float(row["t0_months"]))
    success_rows = [row for row in ordered_rows if bool(row["scp_success"]) and np.isfinite(row["final_miss_km"])]

    print("\n=== Warning-time sweep summary ===")
    if len(success_rows) >= 2:
        first = success_rows[0]
        last = success_rows[-1]

        if float(last["final_miss_km"]) > float(first["final_miss_km"]):
            miss_text = "Earlier intervention increased final miss in this sweep."
        elif float(last["final_miss_km"]) < float(first["final_miss_km"]):
            miss_text = "Earlier intervention reduced final miss in this sweep."
        else:
            miss_text = "Final miss was essentially unchanged across the sweep endpoints."
        print(
            f"{miss_text} "
            f"({first['t0_months']:.0f} mo: {first['final_miss_km']:.3f} km, "
            f"{last['t0_months']:.0f} mo: {last['final_miss_km']:.3f} km)"
        )

        if float(last["iterations_completed"]) < float(first["iterations_completed"]):
            iter_text = "Convergence appeared easier for larger warning times."
        elif float(last["iterations_completed"]) > float(first["iterations_completed"]):
            iter_text = "Convergence did not appear easier for larger warning times."
        else:
            iter_text = "Iteration count stayed roughly unchanged across the sweep endpoints."
        print(
            f"{iter_text} "
            f"({first['t0_months']:.0f} mo: {int(first['iterations_completed'])}, "
            f"{last['t0_months']:.0f} mo: {int(last['iterations_completed'])})"
        )

        if np.isfinite(first["bang_bang_fraction"]) and np.isfinite(last["bang_bang_fraction"]):
            if float(last["bang_bang_fraction"]) < float(first["bang_bang_fraction"]):
                bang_text = "Control became less aggressive as warning time increased."
            elif float(last["bang_bang_fraction"]) > float(first["bang_bang_fraction"]):
                bang_text = "Control became more aggressive as warning time increased."
            else:
                bang_text = "Control aggressiveness was essentially unchanged across the sweep endpoints."
            print(
                f"{bang_text} "
                f"({first['t0_months']:.0f} mo: {first['bang_bang_fraction']:.3f}, "
                f"{last['t0_months']:.0f} mo: {last['bang_bang_fraction']:.3f})"
            )
        else:
            print("Control aggressiveness trend could not be evaluated from the successful runs.")
    else:
        print("Not enough successful SCP runs were available to summarize the warning-time trend.")


def run_warning_time_sweep(
    base_config: VirtualThrustConfig | None = None,
    t0_months_list: tuple[int, ...] | list[int] = WARNING_TIME_SWEEP_MONTHS,
) -> list[dict[str, Any]]:
    """Run the warning-time sweep while keeping all non-warning settings fixed."""

    base_cfg = VirtualThrustConfig() if base_config is None else replace(base_config)
    sweep_months = tuple(int(value) for value in t0_months_list)
    max_t0_months = max(sweep_months)
    required_lead_years = int(np.ceil(max_t0_months / 12.0))
    env_margin_years = max(0.0, float(base_cfg.env_years) - float(base_cfg.lead_time_years))
    sweep_cfg = replace(
        base_cfg,
        lead_time_years=max(int(base_cfg.lead_time_years), required_lead_years),
        env_years=max(float(base_cfg.env_years), max(int(base_cfg.lead_time_years), required_lead_years) + env_margin_years),
    )
    output_root = Path(__file__).resolve().parent / "results" / "warning_time_sweep"
    summary_plot_dir = output_root / "plots"
    output_root.mkdir(parents=True, exist_ok=True)
    summary_plot_dir.mkdir(parents=True, exist_ok=True)

    print(f"Warning-time sweep outputs: {output_root}", flush=True)

    rows: list[dict[str, Any]] = []
    summary_figures: dict[str, go.Figure] = {}

    for t0_months in sweep_months:
        run_cfg = replace(
            sweep_cfg,
            t0_months=int(t0_months),
            show_plots=False,
            print_scp_progress=False,
            run_scp=True,
        )
        run_dir = output_root / f"t0_{int(t0_months):02d}_months"
        run_plot_dir = run_dir / "plots"
        run_dir.mkdir(parents=True, exist_ok=True)
        run_plot_dir.mkdir(parents=True, exist_ok=True)

        print(f"\nRunning warning-time sweep case: t0 = -{int(t0_months)} months", flush=True)
        start_time = time.perf_counter()
        try:
            result = run_virtual_thrust(run_cfg)
        except Exception as exc:
            runtime_s = time.perf_counter() - start_time
            row = {
                "asteroid_name": run_cfg.asteroid_name,
                "n_impactors": int(run_cfg.n_impactors),
                "mass_sc_kg": float(run_cfg.mass_sc_kg),
                "vrel_use_mps": float(run_cfg.vrel_use_mps),
                "cadence_days": float(run_cfg.cadence_days),
                "n_segments": int(run_cfg.n_segments),
                "kmax": int(run_cfg.kmax),
                "force_impact": bool(run_cfg.force_impact),
                "t0_months": int(t0_months),
                "final_miss_km": np.nan,
                "iterations_completed": 0,
                "scp_success": False,
                "scp_converged": False,
                "runtime_s": float(runtime_s),
                "accepted_steps": 0,
                "close_approach_distance_km": np.nan,
                "sampled_ca_km": np.nan,
                "sampled_moid_km": np.nan,
                "max_control_norm": np.nan,
                "mean_control_norm": np.nan,
                "bang_bang_fraction": np.nan,
                "tau_used_s": np.nan,
            ...
            rows.append(row)
            print(
                f"Finished t0 = -{int(t0_months)} months | "
                f"success = False | runtime = {row['runtime_s']:.2f} s | error = {row['error_message']}",
                flush=True,
            )
            continue
        ...
        _save_figure_bundle(result.figures, run_plot_dir)

        control_metrics = _compute_control_usage_metrics(result)
        scp_iterations = 0 if result.scp is None else len(result.scp.iterations)
        scp_success = bool(result.scp is not None and result.scp.success and result.scp.miss_opt_km is not None)
        scp_converged = bool(result.scp is not None and result.scp.converged)
        final_miss_km = float(result.scp.miss_opt_km) if scp_success else np.nan

        row = {
            "asteroid_name": result.asteroid.name,
            "n_impactors": int(run_cfg.n_impactors),
            "mass_sc_kg": float(run_cfg.mass_sc_kg),
            "vrel_use_mps": float(run_cfg.vrel_use_mps),
            "cadence_days": float(run_cfg.cadence_days),
            ...
            "force_impact": bool(run_cfg.force_impact),
            "t0_months": int(t0_months),
            "final_miss_km": final_miss_km,
            "iterations_completed": int(scp_iterations),
            "scp_success": scp_success,
            "scp_converged": scp_converged,
            "runtime_s": float(runtime_s),
            "accepted_steps": 0 if result.scp is None else int(result.scp.accepted_steps),
            "close_approach_distance_km": float(result.close_approach_distance_km),
            "sampled_ca_km": float(result.ca.d_km),
            "sampled_moid_km": float(result.moid.d_km),
            "error_message": "",
            **control_metrics,
        }
        rows.append(row)
        print(
            f"Finished t0 = -{int(t0_months)} months | "
            f"miss = {row['final_miss_km']:.6f} km | "
            f"iterations = {row['iterations_completed']} | "
            f"success = {row['scp_success']} | runtime = {row['runtime_s']:.2f} s",
            flush=True,
        )

    rows.sort(key=lambda row: float(row["t0_months"]))
    _write_sweep_csv(rows, output_root / "warning_time_sweep_results.csv")

    if rows:
        first_row = rows[0]
        info_lines = [
            f"Asteroid: {first_row['asteroid_name']}",
            f"Impactors: {first_row['n_impactors']}",
            f"Segments: {first_row['n_segments']}",
            f"Cadence: {first_row['cadence_days']:.1f} days",
        ]
        summary_figures = {
            "final_miss_vs_t0": _build_sweep_metric_figure(
                rows,
                y_key="final_miss_km",
                y_title="Final miss [km]",
                title=f"Final Miss vs Warning Time | {first_row['asteroid_name']}",
                trace_name="Final miss",
                info_lines=info_lines,
            ),
            "iterations_vs_t0": _build_sweep_metric_figure(
                rows,
                y_key="iterations_completed",
                y_title="SCP iterations completed",
                title=f"SCP Iterations vs Warning Time | {first_row['asteroid_name']}",
                trace_name="Iterations",
                info_lines=info_lines,
            ),
            "bang_bang_fraction_vs_t0": _build_sweep_metric_figure(
                rows,
                y_key="bang_bang_fraction",
                y_title="Fraction of samples with control >= 90% of run max [-]",
                title=f"Control Aggressiveness vs Warning Time | {first_row['asteroid_name']}",
                trace_name="Bang-bang fraction",
                info_lines=info_lines,
            ),
        }
        _save_figure_bundle(summary_figures, summary_plot_dir)
        if base_cfg.show_plots:
            for fig in summary_figures.values():
                fig.show(config={"edits": {"legendPosition": True}})

    _print_warning_time_summary(rows)
    return rows
...
def _build_arg_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--single-run",
        action="store_true",
        help="Run one baseline case instead of the default warning-time sweep.",
    )
    return parser
...
def main() -> None:
    ...
    if not args.single_run:
        run_warning_time_sweep(VirtualThrustConfig())
        return
