...
def test_minimal_interception_chain_propagates_and_prepares_mbi():
    ...
    np.testing.assert_allclose(mbi_states[0].state_earth[:3], mbi_states[0].state_ast[:3], rtol=1e-12, atol=1e-12)
    np.testing.assert_allclose(state_at_moid.state_earth[:3], state_at_moid.state_ast[:3], rtol=1e-12, atol=1e-12)
